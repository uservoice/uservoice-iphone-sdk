...
// before or after the UVTextEditor expands or contracts its height to match.  Obviously,
...
@interface UVTextEditorInternal : NSObject <UITextViewDelegate, UITextFieldDelegate> {
...
- (id)initWithTextEditor:(UVTextEditor*)textEditor;

@end

@implementation UVTextEditorInternal
...
- (void)constrainToText {
    ...
    if (oldHeight && diff) {
        ...
        self.frame = UVRectContract(self.frame, 0, -diff);
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        _delegate = nil;
        _internal = [[UVTextEditorInternal alloc] initWithTextEditor:self];
        _textView = nil;
...
- (void)setDelegate:(id<UVTextEditorDelegate>)delegate {
    _delegate = delegate;
